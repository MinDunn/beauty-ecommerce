...
interface FilterSectionProps {
  title: string;
  type: 'brand' | 'price' | 'skinType';
  items: { id: string; label: string; count?: number }[];
  selectedItems: string[] | string | null;
  onToggle: (type: 'brand' | 'price' | 'skinType', value: string) => void;
  defaultOpen?: boolean;
}
      ...
      <div className={cn("mt-4 space-y-3.5 overflow-hidden transition-all duration-300 origin-top", isOpen ? "max-h-[500px] opacity-100 scale-y-100" : "max-h-0 opacity-0 scale-y-0")}>
        {items.map((item) => {
          const isSelected = Array.isArray(selectedItems) 
            ? selectedItems.includes(item.id || item.label) 
            ...
            <label key={item.id} className="flex items-center group cursor-pointer">
              <div className="relative flex items-center justify-center">
                <input 
                  type="checkbox" 
                  checked={isSelected}
                  onChange={() => onToggle(type, item.id || item.label)}
                  className={cn(
                    "peer w-5 h-5 appearance-none border-2 border-gray-200 transition-colors cursor-pointer outline-none",
                    type === 'price' ? "rounded-full" : "rounded-lg",
                    "checked:bg-primary-500 checked:border-primary-500 hover:border-primary-300"
                  )} 
                />
                <div className={cn(
                  "absolute inset-0 m-auto rounded-sm bg-white scale-0 peer-checked:scale-100 transition-transform pointer-events-none",
                  type === 'price' ? "w-2 h-2 rounded-full" : "w-2.5 h-2.5 rounded-sm"
                )}></div>
              </div>
              <span className={cn("ml-3 text-sm font-medium transition-colors", isSelected ? "text-primary-600 font-bold" : "text-gray-700 group-hover:text-primary-600")}>
                {item.label}
              </span>
...
interface FilterSidebarProps {
  ...
  onFilterChange: (type: 'brand' | 'price' | 'skinType', value: string) => void;
  onReset: () => void;
}
    ...
    <>
      ...
      <div className={cn(
        ...
        <div className="p-4 border-t border-gray-100 bg-white sticky bottom-0 lg:hidden flex gap-3 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)]">
           <button 
             className="flex-1 py-4 bg-gray-50 text-gray-700 font-bold rounded-xl hover:bg-gray-100 transition-colors uppercase tracking-wider text-sm" 
             onClick={() => { onReset(); setIsMobileOpen(false); }}
           >
             Mặc định
           </button>
           <button 
             className="flex-1 py-4 bg-primary-500 text-white font-black rounded-xl shadow-lg shadow-primary-500/30 hover:bg-primary-600 transition-all hover:-translate-y-1 block uppercase tracking-wider text-sm" 
             onClick={() => setIsMobileOpen(false)}
           >
             Áp Dụng
           </button>
