const categories = [
  ...
  { id: 7, name: 'Trang điểm', icon: '💄', color: 'bg-red-50 text-red-500' },
        ...
        <div className="flex overflow-x-auto pt-4 pb-6 hide-scrollbar -mx-4 px-4 md:mx-0 md:px-0 gap-4 md:gap-8 justify-start lg:justify-between snap-x">
          {categories.map((cat) => (
            <div key={cat.id} className="flex flex-col items-center min-w-[80px] md:min-w-[100px] group cursor-pointer snap-start">
              <div className={`w-16 h-16 md:w-24 md:h-24 ${cat.color} rounded-full flex items-center justify-center text-2xl md:text-4xl mb-3 shadow-sm border border-gray-100 group-hover:border-primary-200 transition-all group-hover:-translate-y-2 group-hover:shadow-lg`}>
                {cat.icon}
              </div>
