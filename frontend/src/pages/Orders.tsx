...
import { VATInvoiceTemplate } from "../components/admin/print/VATInvoiceTemplate";
import { PackingSlipTemplate } from "../components/admin/print/PackingSlipTemplate";
...
export const Orders = () => {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `Hoa_don_GLW${orderToPrint?.id || ''}`,
  });

  const handlePrintPacking = useReactToPrint({
    contentRef: packingSlipRef,
    documentTitle: `Phieu_xuat_kho_GLW${orderToPrint?.id || ''}`,
  });
  ...
  return (
    <div className="space-y-6 relative pb-20">
        ...
        <Modal onClose={() => setShowDetails(false)} title={`Đơn hàng #${selectedOrder.id}`}>
          <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
            ...
            <div className="bg-slate-800/30 rounded-3xl p-6 border border-slate-700/50">
               <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-4">Tài liệu & In ấn</p>
               <div className="flex flex-wrap gap-3">
                  ...
                  >
                    <Printer size={14} /> In hóa đơn VAT
                  </button>
                  <button 
            ...
            {/* Quick Actions (Status Dependent) */}
      ...
      <div style={{ display: 'none' }}>
        {orderToPrint && (
          <>
            <VATInvoiceTemplate ref={printComponentRef} order={orderToPrint} />
            <PackingSlipTemplate ref={packingSlipRef} order={orderToPrint} />
          </>
        )}
      </div>
    </div>
