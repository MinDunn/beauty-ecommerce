  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="Tổng doanh thu" value={`${(stats?.totalRevenue / 1000000).toFixed(1)}M`} change="+12.5%" icon={DollarSign} trend="up" />
        <StatCard title="Đơn hàng" value={stats?.totalOrders.toString()} change="+8.2%" icon={ShoppingBag} trend="up" />
        <StatCard title="Khách hàng" value={stats?.totalCustomers.toString()} change="+5.4%" icon={Users} trend="up" />
        <StatCard title="Phản hồi" value={stats?.totalFeedback.toString()} change="+14.1%" icon={MessageSquare} trend="up" />
      </div>
        ...
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 p-8 rounded-[2rem] shadow-xl">
          ...
          <div className="h-64 relative flex items-end gap-2 md:gap-4 px-2 font-sans">
             {stats?.revenueHistory.map((item: any, i: number) => {
               const maxRevenue = Math.max(...stats?.revenueHistory.map((r: any) => r.revenue)) || 1;
               ...
               return (
                <div key={i} className="flex-1 flex flex-col items-center gap-4 group">
                  ...
                  <span className="text-[10px] font-black text-slate-600 group-hover:text-primary-500 transition-colors uppercase tracking-widest">{item.date}</span>
                </div>
               );
             })}
          </div>
